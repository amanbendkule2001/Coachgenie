...
export default function TimetablePage() {
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                ...
                mode: form.location.toLowerCase().includes("online") ? "Online" : "Offline",
                location: form.location,
            };

            if (editing) {
                const updated = await updateOne("timetable", editing.id, payload);
                setSlots(prev => prev.map(s => s.id === editing.id ? updated : s));
            } else {
                const created = await createOne("timetable", payload);
    ...
    slots.forEach((s) => {
        const label = to12Hour(s.start_time);
        if (!slotsByDayTime[s.day]) slotsByDayTime[s.day] = {};
        slotsByDayTime[s.day][label] = s;
    });
                                                ...
                                                {cls ? (
                                                    <div className={clsx("rounded-xl border px-2 py-2 text-left group relative", getColor(cls.id))}>
                                                        ...
                                                        <div className="absolute top-1 right-1 hidden group-hover:flex gap-0.5">
                                                            <button onClick={() => openEdit(cls)} className="p-0.5 rounded hover:bg-white/20" title="Edit class">
                                                                <Pencil size={10} />
                                                            </button>

                                                            {deleteId === cls.id ? (
                                                                <>
                                                                    <button onClick={() => handleDelete(cls.id)} className="p-0.5 rounded bg-danger-500 text-white text-[10px] px-1" title="Confirm delete">Y</button>
                                                                    <button onClick={() => setDeleteId(null)} className="p-0.5 rounded hover:bg-white/20 text-[10px]" title="Cancel delete">N</button>
                                                                </>
                                                            ) : (
                                                                <button onClick={() => setDeleteId(cls.id)} className="p-0.5 rounded hover:bg-white/20" title="Delete class">
                                                                    <Trash2 size={10} />
                                                                </button>
                                                            )}
                                                        </div>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select value={form.day} onChange={e => setForm(p => ({ ...p, day: e.target.value }))} className="input-field" title="Select a day">
                        ...
                        <div>
                            ...
                            <select value={form.class_type} onChange={e => setForm(p => ({ ...p, class_type: e.target.value }))} className="input-field" title="Select class type">
                        ...
                        <div className="col-span-2">
                            ...
                            <input value={form.subject} onChange={e => setForm(p => ({ ...p, subject: e.target.value }))} className="input-field" placeholder="Enter subject" title="Subject" />
                        </div>

                        <div>
                            <label className="block text-xs font-semibold text-text-secondary mb-1.5 uppercase tracking-wide">Start Time</label>
                            <input type="time" value={form.start_time} onChange={e => setForm(p => ({ ...p, start_time: e.target.value }))} className="input-field" title="Start Time" />
                        </div>

                        <div>
                            <label className="block text-xs font-semibold text-text-secondary mb-1.5 uppercase tracking-wide">End Time</label>
                            <input type="time" value={form.end_time} onChange={e => setForm(p => ({ ...p, end_time: e.target.value }))} className="input-field" title="End Time" />
                        </div>

                        <div className="col-span-2">
                            <label className="block text-xs font-semibold text-text-secondary mb-1.5 uppercase tracking-wide">Location</label>
                            <input value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))} className="input-field" placeholder="Enter location" title="Location" />
                        </div>
